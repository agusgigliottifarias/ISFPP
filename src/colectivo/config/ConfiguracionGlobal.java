...
public final class ConfiguracionGlobal {
    ...
    private ConfiguracionGlobal() {
        ...
        // 2. Cargar el ResourceBundle (i18n)
        try {
            String idiomaConfig = props.getProperty("idioma.actual", "es_ES"); // Default "es_ES"
            
            // --- LÓGICA CORREGIDA (Opción 2) ---
            // Esto maneja ambos casos: "en" y "en_US"
            String[] partes = idiomaConfig.split("_");
            Locale locale;
            if (partes.length == 1) {
                // Caso 1: Solo idioma (ej: "en")
                locale = new Locale(partes[0]);
            } else {
                // Caso 2: Idioma y País (ej: "es_ES")
    ...
    /**
     * Devuelve el paquete de recursos (i18n) cargado.
     */
