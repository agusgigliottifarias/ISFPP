...
public class TestCalculo {
    ...
    void setUp() {
        ...
        // Paradas simuladas (IDs como en el modelo)
    ...
    void testRutaDirecta() {
        ...
        // Buscar una alternativa que sea un único segmento (línea A)
        ...
        // Comprobación de duración esperada para la alternativa directa (5+5 = 10)
    ...
    void testRutaConConexionVehicular() {
        // 1 -> 4 por conexión vehicular: A (1->3) luego B (3->4)
    ...
    void testRutaConConexionPeatonal() {
        // Eliminamos conexión directa vehicular (3->4)
        tramos.remove("B-3-4");

        // Agregamos una nueva línea B que sale desde la parada caminada (4)
        tramos.put("B-4-5", new Tramo("B", "4", "5", 8)); // Línea B desde 4 hasta 5

        // Calculamos recorrido 1 -> 5 (colectivo + caminar + colectivo)
        ...
        boolean hayConexionPeatonal = resultados.stream()
            .anyMatch(lista -> lista.size() == 3
                    && lista.get(0).getLinea() != null
                    && lista.get(1).getLinea() == null   // tramo caminando
    ...
    @Test
    void testSinRuta() {
        // Dejamos solo un tramo que no conecta con la parada destino
        tramos.clear();
        tramos.put("A-1-2", new Tramo("A", "1", "2", 5));
