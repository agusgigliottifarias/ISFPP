    ...
    public Map<String, Tramo> buscarTodos() {
        ...
        synchronized (this) {
            ...
            String sql = 
                ...
                "   t.tiempo AS duracion_segundos " + // (Opcional: cambié el alias aquí para claridad)
                ...
                try (PreparedStatement ps = c.prepareStatement(sql)) {
                    ...
                    try (ResultSet rs = ps.executeQuery()) {
                        while (rs.next()) {
                            ...
                            // Leer el valor en segundos y convertirlo a minutos (división entera)
                            int duracionEnSegundos = rs.getInt("duracion_segundos"); 
                            int duracionEnMinutos = duracionEnSegundos / 60; // ¡SOLUCIÓN!

                            Tramo tr = new Tramo(idLinea, origen, destino, duracionEnMinutos); // Pasar minutos
                            // <<<<<<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
