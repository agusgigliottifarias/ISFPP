...
/**
 * Implementación secuencial de FrecuenciaDAO.
 * Lee desde archivo, soporta formato: "COD_LINEA;DIA_SERVICIO;MINUTOS" (ej: L1I;1;15 o L1I;3;06:00:30).
 * Implementa la bandera de carga única (caching thread-safe).
 */
public class FrecuenciaSecuencialDAO implements FrecuenciaDAO {
    ...
    public List<Frecuencia> buscarTodos() {
        ...
        synchronized (this) {
            ...
            for (String s : filas) {
                ...
                // Esperamos al menos 3 columnas: CÓDIGO;DÍA;MINUTOS
                if (a.length < 3) {
    ...
    /** * CORRECCIÓN DE PARSEO: Ahora soporta HH:mm:ss y HH:mm. Ignora los segundos para el cálculo en minutos totales.
     */
    private static int parseMinutos(String v, String lineaOriginal) {
        if (v.contains(":")) {
            ...
            // Lógica para manejar HH:mm:ss (ignora los segundos) o HH:mm
            if (partes.length >= 2) {
            ...
            try {
                ...
                return h * 60 + m; // Minutos totales desde la medianoche
            } catch (NumberFormatException ex) {
                throw new IllegalStateException("Hora inválida o minutos fuera de rango en: " + lineaOriginal, ex);
            }
        }
        
        // Si no contiene ':', intenta parsear como minutos enteros (ej: 180)
        try {
