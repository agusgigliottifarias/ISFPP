...
public class ConsultaController {
    ...
    private void onCalcular() {
        ...
        if (alternativas == null || alternativas.isEmpty()) {
            // Diagnóstico más útil: ¿hay tramos saliendo del origen?
        ...
        if (mainController != null && mainController.getResultadoController() != null) {
            mainController.getResultadoController().setResultados(alternativas.get(0));
...
public class ConsultaController {
    ...
    private void initialize() {
        ...
        // Poblamos horas 00..23 y minutos 00..59
        cbHora.setItems(FXCollections.observableArrayList(
                java.util.stream.IntStream.rangeClosed(00, 23).boxed().collect(Collectors.toList())));
        cbMinuto.setItems(FXCollections.observableArrayList(
                java.util.stream.IntStream.rangeClosed(00, 59).boxed().collect(Collectors.toList())));
        cbHora.getSelectionModel().select(Integer.valueOf(00));  // default 08
        cbMinuto.getSelectionModel().select(Integer.valueOf(00)); // default 00
    ...
    private void onCalcular() {
        ...
        if (alternativas == null || alternativas.isEmpty()) {
            // Mini diagnóstico para que no te deje a ciegas
            int cantParadas = coordinador.getParadas() != null ? coordinador.getParadas().size() : -1;
            int cantTramos  = coordinador.getTramos()  != null ? coordinador.getTramos().size()  : -1;
            new Alert(Alert.AlertType.INFORMATION,
                    "No hay recorridos para esa combinación.\n" +
                    "Paradas cargadas: " + cantParadas + " | Tramos cargados: " + cantTramos + "\n" +
                    "Probá otra dupla o verificá que existan tramos en la misma línea.").showAndWait();
            return;
        }

        // Preview en tabla (primera alternativa)
        if (tblPreview != null) {
            tblPreview.getItems().setAll(alternativas.get(0));
        }

        // Mostrar vista Resultado si la tenés
        if (mainController != null && mainController.getResultadoController() != null) {
            mainController.getResultadoController().setResultados(alternativas.get(0));
            mainController.mostrarResultado();
        }
